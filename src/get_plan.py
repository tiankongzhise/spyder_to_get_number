...
class GetDanZhaoServer(object):
    ...
    def get_jhrs(self,zyzdm:str|int,yxdh:str|int,jhlb:str)->dict|None:
        # 目标URL
    ...
    def get_zyzdm(self,yxdh:str|int,jhlb:str)->dict|None:
        # 目标URL
        ...
        print(f'计划类别{jhlb},院校代号:{yxdh}开始查询专业组')
        return self.query(url,headers,params)
    
    def save_majors_club(self,data_list):
        if self.session is None:
            self.create_session()
        try:
            self.session.bulk_insert_mappings(MajorClubTable, data_list)
            self.session.commit()
            print(f"成功插入")
            return True
        except Exception as e:
